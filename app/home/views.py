...
from flask import render_template, url_for, redirect, flash, session, request,jsonify,g
...
def collect():
    ...
    collect = Collect.query.filter_by(              # 判断是否该收藏
        user_id =int(user_id),
        song_id=int(song_id)
    ).count()
    res = {}
...
def collectList():
    page = request.args.get('page',type=int)  # 获取page参数值
    page_data = Collect.query.filter_by(user_id=g.current_user.id).paginate(page=page, per_page=10)
